...
var onboardingFormStatus = require('../../reducers/users/onboardingFormStatus');
...
var worker = function worker(data, progress, resolve, reject) {
  ...
  axios.get('https://graph.facebook.com/v2.7/me', {
    ...
    if (response.data.birthday) {
      // Check date here because Facebook can be an ass
    ...
    utils.getUserRef(data.payload.uid).child('info').update(basicInfo).then(function () {
      resolve();
    });
...
var key = 'USER_SET_FACEBOOK_TOKEN';
module.exports = {
  worker: worker,
  key: key
};